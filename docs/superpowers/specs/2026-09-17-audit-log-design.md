...
- **Precedence when roles overlap: guild owner, then approver, then bot admin.** `isOwnerOrApprover` checks in that order, so a member who happens to hold a more specific, genuinely-held role in a given guild is attributed as that role rather than as `bot_admin`. Bot admin becomes a true last resort — it only applies when the acting member has no native permission (owner or approver) in that particular guild. This means every check now does a guild lookup and an approver lookup before the bot-admin fallback, rather than the bot-admin shortcut skipping both; in practice the guild lookup is a cheap `Session.State.Guild` cache hit (the bot already has `GUILD_CREATE` data for any guild it's active in), and the approver lookup is a lookup this code path made anyway for non-admins.
